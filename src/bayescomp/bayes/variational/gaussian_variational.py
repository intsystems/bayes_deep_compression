...
class BayesVarNormModule(BaseBayesVarModule):
    ...
    def init_posterior_params_name(self) -> nn.ParameterDict:
        ...
        return nn.ParameterDict(
            {"loc": nn.ParameterList(), "log_scale": nn.ParameterList()}
        )
    ...
    def init_posterior_params(self, p, posterior_params: nn.ParameterDict) -> None:
        ...
        posterior_params.log_scale.append(
            nn.Parameter(-10 + torch.rand_like(p), requires_grad=True)
        )
    ...
    def prune_condition(
        self, loc: torch.Tensor, log_scale: torch.Tensor, threshold: float
    ) -> torch.Tensor:
        # as suggeseted by Graves
        return torch.abs(loc / torch.exp(log_scale)) >= 0.83
