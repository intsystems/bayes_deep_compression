...
class MakeBayessian(nn.Module):
    ...
    def __new__(cls, net: nn.Module):
        ...
        # we remove "MakeBayessian" and "RenamedBayessianModule" from __qualname__
    ...
    def __init__(self, net: nn.Module) -> None:
        ...
        if isinstance(self._net._net, nn.Linear):
            def linear_pre_hook(lin_module: nn.Linear, lin_input: torch.Tensor):
                ...
                lin_module._temp_std_transform = \
                    F.linear(lin_input, self.get_parameter(self._to_std_param_name("weight")))
            ...
            def linear_post_hook(lin_module: nn.Linear, lin_output: torch.Tensor):
                ...
                lin_output = lin_output + torch.randn(lin_output.shape[-1]) * lin_module._temp_std_transform + \
                    torch.randn(lin_output.shape[-1]) * self.get_parameter(self._to_std_param_name("bias"))
    ...
    def sample_estimation(self, x: torch.Tensor, n_samples: int) -> torch.Tensor:
        ...
        """
        posterior_samples = [for _ in range(n_samples) super().__call__(x).unsqueeze(dim=0)]
    ...
    def get_map_module(self, prune: bool = False) -> nn.Module:
        ...
        self._populate_with_map_estimate()
    ...
    def _populate_with_map_estimate(self):
        ...
        for b_m_name, bayes_module in self.named_modules():
            ...
            bayes_module._populate_with_map_estimate()
    ...
    def _sample_params(self):
        ...
        """
        # nn.Linear samples during forward()
        if isinstance(self._net._net, nn.Linear):
            return
        ...
        for p_name, par in self._net.named_parameters(recurse=False):
            ...
            sampled_par += torch.randn_like(sampled_par) * self.get_parameter(self._to_std_param_name(p_name))
