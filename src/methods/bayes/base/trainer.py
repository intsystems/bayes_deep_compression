...
class BaseBayesTrainer(Generic[ModelT], ABC):
    ...
    def __init__(
        ...
        Args:
            self.params: trianing params that is used to fine-tune training
            self.report_chain: All callback that should be return by each epoch
            train_dataset: Dataset on which model should be trained
            eval_dataset: Dataset on which epoch of training model should be evaluated
        """
        self.params = params
